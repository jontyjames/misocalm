...
import { CHECK_IN_SCALES } from '@/lib/constants';
import { getCheckInBackRoute, normalizeCheckInOrigin, normalizeCheckInSource } from '@/lib/checkInContext';
import { useCheckInForm, useReducedMotion } from '@/hooks';
import { SACRED_GLASS_CLASSES, GLASS_HIGHLIGHT_STYLE } from '@/lib/sacredGlass';

export default function BreathworkCheckIn({ userId, fromBreathwork = false, source = null, origin = null }) {
  const router = useRouter();
  const prefersReduced = useReducedMotion();
  const checkInSource = normalizeCheckInSource(source || fromBreathwork);
  const checkInOrigin = normalizeCheckInOrigin(origin);
  const backRoute = getCheckInBackRoute(checkInSource, checkInOrigin);
  const {
    energy, setEnergy,
    ...
    handleSave,
    saving, error,
  } = useCheckInForm(userId, checkInSource, checkInOrigin);
  ...
  return (
    ...
    >
      ...
      {error && (
        <p className="text-sm text-rose-400 font-light mb-4">
      ...
      <div className="mt-auto flex flex-col items-center gap-4">
        ...
        >
          ...
          <span
            ...
            style={{ fontFamily: "'Josefin Sans', sans-serif", fontWeight: 200 }}
          >
            {saving ? 'Saving gently...' : 'Save check-in'}
          </span>
        </button>
